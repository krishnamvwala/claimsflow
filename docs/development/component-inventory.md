...
| --- | --- | --- | --- | --- | --- |
| Python package and CLI | `src/claimsflow` | Python 3.12 | Phase 2 local ingestion implemented | Configuration, generator, provenance gate, local ingestion, and safe control-plane output; no analytics rules | Ruff, strict mypy, pytest |
...
| Python cloud adapters | Future adapters under `src/claimsflow/adapters` | BigQuery 3.43.0; Storage 3.1.1 | Deferred | Future verified landing/raw I/O only; no dbt-owned business logic | Future fake/integration tests |
...
Phase 2 now has its deterministic source generator and local ingestion proof. The next Phase
2 slice will implement Cloud Storage landing and BigQuery raw/audit adapters behind the same
ports, recheck object generations and checksums in the cloud-side task, and prove behavior in
an isolated synthetic dev/demo environment. Cross-family relationship/freshness validation,
governed metrics, dashboards, and automated production deployment remain out of this slice.
