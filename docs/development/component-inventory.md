...
| dbt Core project | `analytics/dbt` | dbt Core 1.12.2; dbt-bigquery 1.12.0 | Phase 4A staging, Phase 4B.1 dimensions, and Phase 4B.2 facts implemented | Fourteen protected typed models read only approved Phase 3 validation IDs; nine protected dimensions preserve conformed history; five protected facts preserve governed grains, parent/effective relationships, ordered diagnoses, lineage, and exact financial controls; no membership publication, metrics, or priority logic | Offline `dbt parse`, generated contract drift, typed and cryptographic staging reconciliation, dimension history/date gates, fact scope/source/relationship/date/diagnosis/financial gates |
...
Phase 4B.2 now has publication-and-selection-isolated claim, claim-line, payment, denial, and
appeal facts with generated contracts, deterministic keys, effective dimension relationships,
ordered diagnosis resolution, exact source totals, and financial-integrity gates. The next
feature slice is Phase 4B.3: immutable candidate membership deltas and safe active-manifest
advancement. A live isolated synthetic GCP exercise remains a separate
