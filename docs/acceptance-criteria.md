...
| AC-ALT-004 | FR-ALT-004 | **Given** quality rates and source ages immediately below, exactly at, and immediately above their configured breach thresholds, **when** monitoring evaluates them, **then** only above-threshold cases alert and each alert reports expected, observed, severity, and affected source or batch. | Quality/freshness below/at/above boundary tests with alert and non-alert assertions | Integration | Blocker | 5 |
...
| Criterion | Requirement | Given / When / Then | Required evidence | Test level | Gate | Phase |
| --- | --- | --- | --- | --- | --- | --- |
| AC-PERF-001 | NFR-PERF-001 | **Given** a synthetic monthly workload of at least 100,000 claims plus related claim lines, payments, denials, and appeals on the versioned reference environment, **when** the full monthly pipeline and a 20-request operational-view test run, **then** the pipeline finishes within 30 minutes, each operational request returns at most 500 records with a warm-cache p95 server response time of at most 5 seconds, each representative operational query scans at most 1 GiB, and browser memory grows by at most 200 MiB during a 30-minute workflow without loading the full dataset. | Pre-run versioned workload and reference-environment manifest plus scale report recording duration, per-query scanned bytes, page sizes, 20-request latency distribution, and browser-memory measurements | Performance | Conditional | 8 |
