...
| AC-ING-004 | FR-ING-004 | **Given** any raw record, **when** its lineage keys are followed, **then** the exact source file and source row or source-record identifier can be retrieved. | Bidirectional lineage query for representative records | Integration | Blocker | 2 |
| AC-ING-005 | FR-ING-005 | **Given** a successfully processed batch, **when** the identical batch is processed again, **then** business-row counts and published financial totals remain unchanged. | Replay test with before-and-after counts and amounts | End-to-end | Blocker | 2 |
...
| AC-PRI-006 | FR-PRI-006 | **Given** claims with critical identity, financial, or deadline failures, **when** scoring runs, **then** they are absent from ranked output and present in a blocking-evidence output. | Negative eligibility tests and anti-join reconciliation | dbt | Blocker | 6 |
...
| Criterion | Requirement | Given / When / Then | Required evidence | Test level | Gate | Phase |
| --- | --- | --- | --- | --- | --- | --- |
| AC-ALT-001 | FR-ALT-001 | **Given** a high-value claim inside the configured filing or appeal window, **when** alert evaluation runs, **then** the claim appears once with amount and days remaining. | Deadline-boundary fixtures and alert result | Integration | Blocker | 6 |
| AC-ALT-002 | FR-ALT-002 | **Given** a payer or denial-category rate crossing its configured spike threshold, **when** alert evaluation runs, **then** an alert identifies the cohort, baseline, observed value, and variance. | Threshold-boundary test and alert evidence | Integration | Blocker | 6 |
...
| AC-ALT-004 | FR-ALT-004 | **Given** a quality rate or source age crossing its configured threshold, **when** monitoring evaluates it, **then** an alert reports expected, observed, severity, and affected source or batch. | Quality/freshness boundary tests | Integration | Blocker | 5 |
| AC-ALT-005 | FR-ALT-005 | **Given** a payment variance outside its documented tolerance, **when** reconciliation runs, **then** an alert identifies the claim or batch, compared amounts, tolerance, and variance. | Financial-variance fixtures and alert result | Integration | Blocker | 6 |
| AC-ALT-006 | FR-ALT-006 | **Given** unresolved claim volume or age crossing its configured backlog threshold, **when** monitoring runs, **then** an alert identifies the affected cohort, threshold, and observed backlog. | Backlog-threshold test and alert result | Integration | Blocker | 6 |
...
| Criterion | Requirement | Given / When / Then | Required evidence | Test level | Gate | Phase |
| --- | --- | --- | --- | --- | --- | --- |
| AC-SEC-001 | NFR-SEC-001 | **Given** all committed datasets, fixtures, reports, and screenshots, **when** provenance is reviewed, **then** every record is generated or explicitly synthetic and no real personal or customer data is present. | Synthetic-data provenance manifest and release review | Security | Blocker | 1/8 |
...
| Criterion | Requirement | Given / When / Then | Required evidence | Test level | Gate | Phase |
| --- | --- | --- | --- | --- | --- | --- |
| AC-PERF-001 | NFR-PERF-001 | **Given** a synthetic workload representing at least 100,000 claims per month, **when** the pipeline and operational views run, **then** processing completes within the approved test budget and clients use bounded/paginated results. | Scale-test report with workload, duration, and client memory/query behavior | Performance | Conditional | 8 |
...
| Criterion | Requirement | Given / When / Then | Required evidence | Test level | Gate | Phase |
| --- | --- | --- | --- | --- | --- | --- |
| AC-AUD-001 | NFR-AUD-001 | **Given** any trusted record, **when** lineage is traversed, **then** its source delivery, batch, and original source record can be identified. | Random-sample lineage traversal test | Integration | Blocker | 4 |
...
| E2E-002 - Duplicate delivery | The same delivered file is detected; replay changes neither trusted row counts nor financial totals. | AC-ING-005, AC-ING-006, AC-REL-001, AC-REL-002 | Duplicate audit decision and before/after comparison |
...
| E2E-004 - Verified correction | A quarantined record is corrected without overwriting raw evidence, revalidated, and published only after all blocking rules pass. | AC-ING-007, AC-DQ-005, AC-DQ-008, AC-DQ-009 | Original/revised audit, post-validation result, and reconciliation |
...
| E2E-007 - Operational failure and recovery | An injected failure blocks publication, records actionable evidence, alerts the engineer, and succeeds on an idempotent retry. | AC-ALT-003, AC-OPS-003 through AC-OPS-006, AC-REL-004 | Failure and recovery run, alert, audit, and unchanged trusted totals |
